...
import { Trash2, Download, Send, RefreshCcw } from 'lucide-react';
import { LogEntry } from '../types';

interface BlePeripheralViewProps {
  logs: LogEntry[];
...
export const BlePeripheralView: React.FC<BlePeripheralViewProps> = ({ 
  logs, 
  onSend, 
  onClear, 
  isAdvertising,
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0d1117] relative">
      {/* Header */}
      <header className="h-16 border-b border-border-dark flex items-center justify-between px-6 bg-surface-dark/50 backdrop-blur-md sticky top-0 z-10 shrink-0">
        <div className="flex items-center gap-4">
          <h2 className="text-white text-xl font-bold tracking-tight">BLE 从机模拟器</h2>
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end mr-2">
            <div className="flex items-center gap-3 bg-[#1c222e] px-3 py-1.5 rounded-lg border border-border-dark">
              <span className="text-xs text-[#9da6b9] font-medium">广播开关</span>
              ...
              <span className="text-xs text-[#9da6b9] font-medium">设备可见性</span>
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  type="checkbox" 
                  className="sr-only peer" 
            ...
            <p className="text-[10px] text-[#565f73] mt-1 mr-1">开启广播后，中心设备可扫描并连接 (Service: FFE0)</p>
          </div>
        </div>
      </header>
      ...
      <div className="h-12 border-b border-border-dark bg-[#161b22] flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-4">
          <div className="bg-[#111318] p-1 rounded-lg border border-[#3b4354] flex gap-1">
            <button 
              ...
              className={`px-3 py-1 rounded text-[11px] font-bold border transition-colors ${viewMode === 'HEX' ? 'bg-[#1c222e] text-white border-[#3b4354] shadow-sm' : 'text-[#9da6b9] border-transparent hover:text-white hover:bg-[#1c222e]/50'}`}
            >
              十六进制视图
            </button>
            <button 
              onClick={() => setViewMode('ASCII')}
              className={`px-3 py-1 rounded text-[11px] font-bold border transition-colors ${viewMode === 'ASCII' ? 'bg-[#1c222e] text-white border-[#3b4354] shadow-sm' : 'text-[#9da6b9] border-transparent hover:text-white hover:bg-[#1c222e]/50'}`}
            >
              ASCII 视图
            </button>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-1 text-[#9da6b9] hover:text-white transition-colors px-2 py-1 rounded hover:bg-[#3b4354]">
            <Download size={14} />
            <span className="text-[11px]">导出数据</span>
          </button>
          <button 
            onClick={onClear}
            className="flex items-center gap-1 text-[#9da6b9] hover:text-red-400 transition-colors px-2 py-1 rounded hover:bg-[#3b4354]"
          >
            <Trash2 size={14} />
      ...
      <div className="flex-1 overflow-y-auto relative bg-[#0d1117] custom-scrollbar" ref={scrollRef}>
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="text-[#9da6b9] text-xs font-medium border-b border-[#282e39] sticky top-0 bg-[#161b22] z-10">
        ...
        <div className="flex gap-3 h-12">
          <div className="relative flex-1 h-full">
            <textarea 
              ...
              placeholder={isAdvertising ? "输入数据通过 FFE1 发送至主机..." : "请先开启广播"}
          ...
          <button 
            onClick={handleSend}
            disabled={!isAdvertising}
            className={`bg-primary hover:bg-blue-600 active:scale-95 text-white px-6 rounded-lg text-sm font-bold shadow-lg shadow-primary/20 transition-all flex items-center justify-center gap-2 shrink-0 h-full w-28 group ${!isAdvertising ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
